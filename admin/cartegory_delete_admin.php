...
if($image_name != ""){
    ...
    if($remove == false){
        ...
        header("location: ./manage_cartegory.php?error=failedToRemoveImage");
        die();
    }

}
// remove from database
$sql = "DELETE FROM tbl_cartegory WHERE id='$id'";
$res = mysqli_query($conn,$sql);
// check if deleted
if ($res){
    $_SESSION['deleted']='<div class="alert alert-success" role "alert"> Successfully deleted </div>';
    header("location: ./manage_cartegory.php?error=successfullyDeleted");
}
else{
    $_SESSION['deleted']='<div class="alert alert-danger" role "alert"> Failed to remove delete </div>';
    header("location: ./manage_cartegory.php?error=NotDeleted");
}
}
